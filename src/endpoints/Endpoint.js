module.exports = class Endpoint {
   ...
   checkRateLimit(ip) {
      ...
      if (this.rateLimit.store[ip] === undefined)
         this.rateLimit.store[ip] = {
            requests: 0,
            lastRequest: Date.now(),
         };
      if (this.rateLimit.store[ip].requests >= this.rateLimit) {
         if (
            Date.now() - this.rateLimit.store[ip].lastRequest <
            this.rateLimit.cooldown
         )
            return true;
   ...
   validate(request) {
      if (this.checkRateLimit(request.ip)) {
         if (this.webserver.config.webserver.debug)
            console.log(`[WEBSERVER] ${request.ip} is rate limited.`);
         return {
            status: 429,
            body:
               "Rate limit exceeded. Please try again later. (Cooldown: " +
               this.rateLimit.cooldown +
               "ms)",
         };
      }
      ...
      if (!this.checkAuthorization(request.headers.authorization)) {
         if (this.webserver.config.webserver.debug)
            console.log(
               `[WEBSERVER] ${request.ip} is unauthorized. Invalid authorization header.`
            );
         return {
            status: 401,
      ...
      if (!this.checkIP(request.ip)) {
         if (this.webserver.config.webserver.debug)
            console.log(
               `[WEBSERVER] ${request.ip} is unauthorized. IP not allowed.`
            );
         return {
            status: 403,
            body: "Forbidden. You are not allowed to access this endpoint.",
         };
      }
   }
   ...
   get(req, res) {
      if (this.webserver.config.webserver.debug)
         console.log(
            `[WEBSERVER] ${req.ip} requested ${req.url}. NOT IMPLEMENTED`
         );
      return {
         status: 405,
   ...
   handle(ctx) {
      ...
      if (validationError) {
         ...
         if (this.webserver.config.webserver.debug)
            console.log(
               `[WEBSERVER] (${ctx.request.method})${ctx.request.ip} requested ${ctx.request.url}. ${validationError.body}`
            );
      ...
      if (ctx.request.method === "GET") {
         if (this.webserver.config.webserver.debug)
            console.log(
               `[WEBSERVER] (${ctx.request.method})${ctx.request.ip} requested ${ctx.request.url}.`
            );
         ...
         if (data) {
            if (this.webserver.config.webserver.debug)
               console.log(
                  `[WEBSERVER] (${ctx.request.method})Response from ${ctx.request.url} to ${ctx.request.ip}: [${data.status}] ${data.body}`
               );
         ...
         if (this.webserver.config.webserver.debug)
            console.log(
               `[WEBSERVER] (${ctx.request.method})${ctx.request.ip} requested ${ctx.request.url}. Method not supported.`
            );
         return {
            status: 405,
            body: "Method not allowed.",
   ...
   static validateOptions(webserver, options) {
      ...
      if (options.allowedIPs) {
         ...
         if (!Array.isArray(options.allowedIPs))
            throw new TypeError(
               "allowedIPs is not an Array. To allow all IPs, remove this property."
            );
      ...
      if (options.ratelimit) {
         if (options.ratelimit.rpm) {
            ...
            if (typeof options.ratelimit.rpm !== "number")
               throw new TypeError(
                  "ratelimit.rpm is not a number. To disable rate limiting, remove this property."
               );
            if (options.ratelimit.rpm < 1)
               throw new Error(
                  'ratelimit.rpm is too low. Must be at least 1. Do disable the endpoint, set the "disabled" property to true.'
               );
         }
         if (options.ratelimit.cooldown) {
            ...
            if (typeof options.ratelimit.cooldown !== "number")
               throw new TypeError(
                  "ratelimit.cooldown is not a number. To disable rate limiting, remove this property."
               );
            if (options.ratelimit.cooldown < 100)
               throw new Error(
                  "ratelimit.cooldown is too low. Must be at least 100 milliseconds"
               );
         }
      }
   }
