...
module.exports = class ServersCommand extends Command {
    ...
    run(message, args) {
        ...
        const servers = [...message.client.guilds.cache.values()]
            ...
            .map(guild => {
                return `\`${guild.id}\` - \`${guild.memberCount}\` - **${guild.name}** - ${moment(guild.me.joinedAt).fromNow()}`;
            });
        ...
        } else new ReactionMenu(message.client, message.channel, message.member, embed, servers);
    }
};
