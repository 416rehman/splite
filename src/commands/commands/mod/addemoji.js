...
async function addEmoji(emoji, message, command, emojiName)
{
  ...
  if (customemoji.id) {
    ...
    try {
      ...
      return message.channel.send({embeds: [new Discord.MessageEmbed().setDescription(`${addedEmoji} added with name "${addedEmoji.name}"`).setFooter(message.member.displayName, message.author.displayAvatarURL({dynamic: true}))]});
    } catch(e) {
      return message.channel.send({embeds: [new Discord.MessageEmbed().setDescription(`${_emojis.fail} Failed to add emoji\n\`\`\`${e.message}\`\`\``).setFooter(message.member.displayName, message.author.displayAvatarURL({dynamic: true}))]});
    }
  }
  else {
