...
module.exports = class MuteCommand extends Command {
  ...
  async run(message, args) {
    ...
    message.channel.send({embeds: [new MessageEmbed().setTitle('Mute Member')
        .setDescription(`Do you want to mute ${member} for **${ms(time, {long: true})}**?`).setFooter(`Expires in 15s`)], components: [row]}).then(async msg=> {
      ...
      collector.on('collect', async b => {
        ...
        if (b.customId === 'proceed') {
          ...
          updated = true;
          msg.edit({ components: [], embeds: [new MessageEmbed().setTitle('Mute Member')
