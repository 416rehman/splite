...
module.exports = class WhoIsCommand extends Command {
   ...
   async run(message, args) {
      ...
      if (this.client.getOwnerFromId(member.user.id))
         userFlags.push("BOT_OWNER");
      if (this.client.getManagerFromId(member.user.id))
         userFlags.push("BOT_MANAGER");
      const activities = [];
      let customStatus;
      ...
      if (KeyPerms.includes("ADMINISTRATOR"))
         KeyPerms.move(
            KeyPerms.findIndex((p) => p === "ADMINISTRATOR"),
            0
         );
      if (
         KeyPerms.includes("MANAGE_GUILD") &&
         KeyPerms.includes("ADMINISTRATOR")
      )
         KeyPerms.move(
            KeyPerms.findIndex((p) => p === "MANAGE_GUILD"),
            1
         );
      else if (KeyPerms.includes("MANAGE_GUILD"))
         KeyPerms.move(
            KeyPerms.findIndex((p) => p === "MANAGE_GUILD"),
            0
         );
      ...
      let roles = message.client.utils.trimArray(
         [...member.roles.cache.values()].filter((r) => !r.name.startsWith("#"))
      );
      ...
      const embed = new MessageEmbed()
         ...
         .addField(
            "Color Role",
            member.roles.color?.toString() || "`None`",
            true
         )
