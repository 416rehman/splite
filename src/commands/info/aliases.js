...
module.exports = class AliasesCommand extends Command {
    constructor(client) {
        super(client, {
            name: 'aliases', aliases: ['alias', 'ali', 'a'], usage: 'aliases [command type]', description: oneLine`
      ...
      `, type: client.types.INFO, examples: ['aliases Fun']
        });
    }
    ...
    run(message, args) {
        ...
        let disabledCommands = this.client.db.settings.selectDisabledCommands
            .pluck()
        ...
        if (args[0] && types.includes(type) && (type !== OWNER || this.client.isOwner(message.member)) && (type !== MANAGER || this.client.isManager(message.member))) {

            this.client.commands.forEach((command) => {
                if (command.aliases && command.type === type && !disabledCommands.includes(command.name) && !command.name.startsWith('clear')) aliases[command.type].push(`**${command.name}:** ${command.aliases
                    .map((a) => `\`${a}\``)
                    .join(' ')}`);
            });
            ...
            else new ButtonMenu(this.client, message.channel, message.member, embed, aliases[type]);
        }
        else if (type) {
            ...
            this.client.commands.forEach((command) => {
                if (command.aliases && !disabledCommands.includes(command.name)) aliases[command.type].push(`**${command.name}:** ${command.aliases
                    .map((a) => `\`${a}\``)
                    .join(' ')}`);
            });
        ...
        `)
            ...
            for (const type of Object.values(this.client.types)) {
                if (type === OWNER && !this.client.isOwner(message.member)) continue;
                if (type === MANAGER && !this.client.isManager(message.member)) continue;
                if (aliases[type][0]) embed.addFields({
                    name:`**${emojiMap[type]}**`,
                    value: `\`${aliases[type].reduce((a, b) => a + b.split(' ').slice(1).length, 0)}\` aliases`,
                    inline: true});
            }
