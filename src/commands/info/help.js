...
module.exports = class HelpCommand extends Command {
   ...
   async run(message, args) {
      ...
      if (
         command &&
         (command.type !== OWNER ||
            message.client.isOwner(message.member) ||
            command.type !== MANAGER ||
            message.client.isManager(message.member)) &&
         ...
         message.client.commands.forEach((command) => {
            if (
               !disabledCommands.includes(command.name) &&
               !command.name.startsWith("clear") &&
               !command.name.startsWith("texthelp")
            ) {
               if (
                  command.type === this.client.types.OWNER &&
                  !message.client.isOwner(message.member)
               )
                  return;
               if (
                  command.type === this.client.types.MANAGER &&
                  !message.client.isManager(message.member)
               )
                  return;
         ...
         commands["Slash Only Commands"] = message.client.slashCommands.map(
            (s) => {
               if (s.name === "view") {
         ...
         commands["Slash Only Commands"] =
            commands["Slash Only Commands"].join("\n");
        ...
        `
            ...
            .addField(
               "**Links**",
               `[Invite Me](${message.client.config.inviteLink}) | [Support Server](${message.client.config.supportServer})`
            );
         ...
         collector.on("collect", (b) => {
            ...
            if (type === "Slash Only Commands")
               tempEmbed.addField(
                  `${emojis.verified_developer} **/${type}**`,
                  "" + commands[type]
               );
            else
               tempEmbed.addField(
                  ...
                  `${
                     emojiMap[type].includes("Admin")
                        ? 'Commands can be cleared by replacing "set" with "clear".\ni.e `setmodlog` ➔ `clearmodlog`\n-----------------------------------------------------\n'
                        : ""
                  } ${commands[type].join(", ")}`
               );
