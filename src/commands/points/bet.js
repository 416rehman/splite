...
module.exports = class betCommand extends Command {
   ...
   async run(message, args) {
      ...
      if (this.instances.has(member.user.id)) {
         ...
         return message
            .reply(
               `${emojis.fail} ${member.user.username} is already betting against someone! Please try again later.`
            )
            .then((m) => {
               setTimeout(() => m.delete(), 5000);
            });
      ...
      if (amount < 0 || amount > otherPoints) {
         ...
         return message
            .reply(
               `${emojis.nep} ${member.user.username} only has ${otherPoints} points ${emojis.point}! Please change your betting amount!`
            )
            .then((m) => {
               setTimeout(() => m.delete(), 5000);
            });
         ...
         message.channel
            ...
            .then((msg) => {
               ...
               collector.on("collect", (b) => {
                  ...
                  if (b.customId === "proceed") {
                     const embed = new MessageEmbed()
                        .setTitle(
                           `${message.author.username} VS ${member.user.username}`
                        )
                        .setDescription(
                           `${emojis.point} **Rolling for ${amount} points** ${emojis.point}\n${emojis.dices}${emojis.dices}${emojis.dices}`
                        )
                        .setFooter({
                     ...
                     message.channel
                        ...
                        .then((msg2) => {
                           ...
                           setTimeout(() => {
                              ...
                              const winnerPoints =
                                 winner.id === member.id ? otherPoints : points;
                              ...
                              const loser =
                                 winner.id === member.id
                                    ...
                                    : member.user;
                              const loserPoints =
                                 winner.id === member.id ? points : otherPoints;
                              ...
                              const embed = new MessageEmbed()
                                 ...
                                 .setDescription(
                                    `🎉 ${winner} has won ${amount} points ${emojis.point} from ${loser}!`
                                 )
                                 .setFooter({
                                    text: `🏆 ${winner.username}'s points: ${
                                       winnerPoints + amount
                                    } | ${loser.username}'s points: ${
                                       loserPoints - amount
                                    }`,
                                 });
                     ...
                     this.done(member.user.id);
                     msg.edit(
                        `${emojis.fail} ${message.author}, ${member.user.username} has rejected your bet!`
                     ).then((msg) => {
               ...
               collector.on("end", () => {
                  ...
                  msg.edit({
                     components: [],
                     content: `${member} did not accept the bet - Expired`,
                  });
               });
            });
      } catch (e) {
