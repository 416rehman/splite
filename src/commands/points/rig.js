...
module.exports = class WipePointsCommand extends Command {
    ...
    run(message) {
        let bal = message.client.db.users.selectPoints
            .pluck()
        ...
        if (bal >= cost) {
            ...
            message.guild.shippingOdds.set(
                message.author.id,
                new Date().getTime()
            );
            ...
            const embed = new MessageEmbed()
                .setTitle('Rig Ship')
                .setDescription(
                    `Successfully rigged ${message.author}'s shipping odds for 30 mins.`
                )
            ...
            return message.reply(
                `${emojis.nep} **You need ${cost - bal} more points ${
                    emojis.point
                } in this server to run this command.**\n\nTo check your points ${
                    emojis.point
