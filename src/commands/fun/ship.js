...
module.exports = class shipCommand extends Command {
   ...
   async run(message, args) {
      const member = await this.getGuildMember(
         message.guild,
         args[0] ||
            this.client.db.users.getRandomWithBio.get(message.guild.id).user_id
      );
      const member2 =
      ...
      let shipOddsTime;
      if (member2.id === message.author.id || member.id === message.author.id)
      ...
      try {
         ...
         message.channel.send({
            embeds: [new MessageEmbed().setDescription(`${emojis.fail} ${e}`)],
         });
      }
   }
