...
module.exports = class ditherCommand extends Command {
  constructor(client) {
    super(client, {
      name: 'dither',
  ...
  async run(message, args) {
    ...
    message.channel.send(new MessageEmbed().setDescription(`${load} Loading...`)).then(async msg=>{
      try {
        const buffer = await msg.client.ameApi.generate("dither", { url: this.getAvatarURL(member) });
        const attachment = new MessageAttachment(buffer, "dither.png");
