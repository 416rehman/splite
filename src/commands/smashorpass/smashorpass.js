...
module.exports = class smashOrPassCommand extends Command {
   ...
   async run(message, args) {
      ...
      const optOutSmashOrPass = message.client.db.users.selectOptOutSmashOrPass
         .pluck()
         .get(message.author.id);
      if (optOutSmashOrPass === 1) {
         const embed = new MessageEmbed()
            .setTitle(
               `${emojis.smashorpass} Smash or Pass ${emojis.smashorpass}`
            )
            .setDescription(
               `To use this command, you must be opted-in to ${emojis.smashorpass} Smash or Pass ${emojis.smashorpass}.\nPlease opt back in, by typing **\`${prefix}toggleSmashOrPass\`**`
            );
      ...
      if (points < cost) {
         ...
         return await message.reply(
            `${emojis.nep} You need **${cost - points}** more points ${
               emojis.point
            } in this server to play ${emojis.smashorpass} **Smash or Pass** ${
               emojis.smashorpass
      ...
      if (args.length) {
         ...
         const seenBefore = message.client.db.SmashOrPass.getSeenByUser.get(
            message.author.id,
            member.user.id
         );
         if (seenBefore) {
            ...
            if (seenBefore.liked == "yes") {
               if (matched) {
                  this.done(message.author.id);
                  return await message.reply(
                     `${emojis.smashorpass} You two have matched already ${emojis.smashorpass}. To unmatch ${emojis.unmatch}, type \`${prefix}unmatch <user mention/id>\``
                  );
               }
               this.done(message.author.id);
               return message.reply(
                  `You already voted 🔥 Smash on ${member.user.username}. To reset your ${emojis.smashorpass} Smash or Pass ${emojis.smashorpass} history, type \`${prefix}resetSmashOrPass\``
               );
            } else {
               this.done(message.author.id);
               return message.reply(
                  `You already voted 👎 Pass on ${member.user.username}. To reset your ${emojis.smashorpass} Smash or Pass ${emojis.smashorpass} history, type \`${prefix}resetSmashOrPass\``
               );
            }
         }

         let bio =
            message.client.db.bios.selectBio.get(member.id).bio ||
         ...
         message.channel
            .send({ embeds: [embed], components: [controls] })
            .then((msg) => {
               const filter = (button) => {
                  button.deferUpdate();
               ...
               handleSmashOrPass
                  .call(this, msg, message.author, points, member, collector)
                  .then(async (result) => {
                     await msg.edit({
                        embeds: [
                           new MessageEmbed()
                              ...
                              .setFooter({
                                 text: `Expires in 30 seconds | Points: ${points}`,
                              }),
                        ],
                     });
                     this.done(message.author.id);
         ...
         const likedByUsers =
            message.client.db.SmashOrPass.getLikedByUsers.all({
               userId: message.author.id,
            }) || [];
         const unseenUsers =
            message.client.db.SmashOrPass.getUnseenUsers.all({
               userId: message.author.id,
            }) || [];
            ...
            message.channel
               ...
               .then(async (msg) => {
                  ...
                  while (points >= cost && usersToBeShown.length) {
                     ...
                     if (currentUser) {
                        try {
                           let result = await handleSmashOrPass.call(
                              this,
                              msg,
                              message.author,
                              points,
                  ...
                  if (points < cost)
                     ...
                     await stopPlaying(
                        msg,
                        message.author.id,
                        `${emojis.fail} Maximum swipes reached per session. Try again later.\nOr\nYou may have viewed everyone. Consider resetting using the command **\`${prefix}resetsmashorpass\`**`
                     );
               });
         } else {
            this.done(message.author.id);
            await message.reply(
               `You have viewed everyone. Consider resetting using the command **\`${prefix}resetsmashorpass\`**`
            );
         }
      }
   }
...
function nextUser(message, usersQueue, points, prefix) {
   return new Promise((resolve, reject) => {
      if (usersQueue.length) {
         ...
         if (guild) {
               ...
               message.edit({
                  embeds: [
                     new MessageEmbed()
                        ...
                        .setImage(
                           currentUser.user.displayAvatarURL({
                              dynamic: true,
                              size: 512,
                           })
                        )
            ...
            reject(`${emojis.fail} Could not find guild ${newUser.guild_id}`);
      } else reject(`${emojis.fail} No more users to view.`);
   });
}
      ...
      collector.on("collect", async (btn) => {
         if (btn.customId === "smash" && btn.user.id === author.id) {
            try {
               ...
               msg.client.db.SmashOrPass.insertRow.run(
                  author.id,
                  currentUser.user.id,
                  "yes",
                  date.toISOString()
               );
               msg.client.db.users.updatePoints.run(
                  { points: -cost },
                  author.id,
                  msg.guild.id
               );
                  ...
                  await author.send({
                     embeds: [
                        new MessageEmbed()
                           ...
                           .setFooter({
                              text: "Remember to always be respectful!",
                           }),
                     ],
                  });
               }
