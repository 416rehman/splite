...
module.exports = class MatchesCommand extends Command {
    ...
    async run(message, args) {
        ...
        if (args.length > 0) {
            ...
            matches = message.client.db.SmashOrPass.getMatches.all(
                member.user.id,
                member.user.id
            );
        ...
        for (const row of matches) {
            const mUser = await message.client.db.users.selectRowUserOnly.get(
                row.shownUserID
            );
            const d = row.dateandtime;
            members.push(oneLine`
        **${count}.** ${mUser.user_name}#${
    mUser.user_discriminator
} - \`${moment(d).fromNow()}\`
      `);
            count++;
        }
        ...
        const embed = new MessageEmbed()
            .setFooter({
                text: `To start fresh, type ${prefix}resetSmashOrPass`,
        ...
        if (members.length <= max) {
            ...
            message.channel.send({
                embeds: [
                    embed
                        .setTitle(
                            `${name}'s ${emojis.smashorpass} Smash Or Pass ${emojis.smashorpass} Matches ${emojis.matches} ${range}`
                        )
            ...
            embed
                .setTitle(
                    `${name}'s ${emojis.smashorpass} Smash Or Pass ${emojis.smashorpass} Matches ${emojis.matches}`
                )
                .setFooter({
