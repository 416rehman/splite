...
module.exports = class reportCommand extends Command {
         ...
         slashCommand: new SlashCommandBuilder()
            .addSubcommand((subcommand) =>
               subcommand
                  .setName("confession")
                  .setDescription(
                     "Report a confession for ToS violation or harmful/hateful content"
                  )
   ...
   interact(interaction, args) {
      if (interaction.options.getSubcommand() === "confession") {
         const reportsChannel = interaction.client.channels.cache.get(
            interaction.client.config.confessionReportsID
         );
         if (reportsChannel)
            reportsChannel.send(
               `${interaction.user.username}#${
                  interaction.user.discriminator
               } (${interaction.user.id}) has reported Confession ID ${
                  args[0].value
               }\n**Reason**\n||*${args[1]?.value || "None"}*||`
            );
         return interaction.reply({
            content: "Your report has been received! Thank you",
            ephemeral: true,
         });
...
function sendBugReport(report, context, isInteraction) {
   ...
   if (!reportChannel)
      return context.reply({
         content:
            "The bug report channel could not be found. Please contact the bot owner.",
      });
      ...
      .setDescription(
        ...
        Successfully sent bug report!
         ${
            ...
            `To further discuss your issue, contact ${this.client.owners[0]}`
         }
      `
      )
