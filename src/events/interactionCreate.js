...
module.exports = async (client, interaction ) => {
    if (interaction.isCommand()) {
        ...
        if (command) {
            ...
            if (cooldown)
                return interaction.reply({embeds: [new MessageEmbed().setDescription(`You are on a cooldown. Try again in **${cooldown}** seconds.`)], ephemeral: true});

            const instanceExists = command.isInstanceRunning(interaction.user.id)
            if (instanceExists)
                return interaction.reply({embeds: [new MessageEmbed().setDescription(`Command already in progress, please wait for it.`)], ephemeral: true})
            ...
            if (permissionErrors) return interaction.reply({embeds: [permissionErrors], ephemeral: true})
