...
module.exports = async (client, message) => {
   ...
   let afkStatus = message.client.db.users.selectAfk.get(
      message.guild.id,
      message.author.id
   );
   ...
   if (afkStatus?.afk != null) {
      ...
      if (message.member.nickname)
         message.member
            ...
            .catch((err) => {
               console.log(
                  "There was an error while trying to remove the AFK tag from the nickname: " +
                     message.author.tag
               );
            });
      message.channel
         .send(
            `${online} Welcome back ${message.author}, you went afk **${moment(
               d
            ).fromNow()}**!`
         )
         .then((msg) => {
   ...
   if (message.mentions.users.size > 0) {
      ...
      message.mentions.users.forEach((user, idx) => {
         ...
         const afkStatus = message.client.db.users.selectAfk.get(
            message.guild.id,
            user.id
         );
         if (afkStatus != null) {
   ...
   if (prefixRegex.test(message.content)) {
      ...
      if (command && !disabledCommands.includes(command.name)) {
         //Blacklisted user
         if (command.checkBlacklist(message.author))
            return message
               .reply({
                  embeds: [
                     new MessageEmbed().setDescription(
                        `${fail} You are blacklisted. Please contact the developer **\`${message.client.ownerTag}\`** for appeals.`
                     ),
                  ],
               })
               .then((msg) => {
         ...
         if (instanceExists)
            return message
               .reply({
                  embeds: [
                     new MessageEmbed().setDescription(
                        `${fail} Command already in progress, please wait for it.`
                     ),
                  ],
               })
               .then((msg) => {
         ...
         if (permissionErrors instanceof MessageEmbed)
            return message.reply({ embeds: [permissionErrors] });
