...
class Client extends Discord.Client {
         ...
         .catch((error) => {
            ...
            if (error.code === 50001)
               return this.logger.error(
                  `Failed to setup slash commands for guild: ${guild}. Missing perms.`
               );
         });
   }
   ...
   registerSlashCommands(guild, commands, id) {
      return new Promise(async (resolve, reject) => {
         ...
         try {
            ...
            await rest.put(Routes.applicationGuildCommands(id, guild.id), {
               body: slashCommands,
            });

            guild.commands.fetch().then(async (registeredCommands) => {
               let fullPermissions = registeredCommands.map(async (c) =>
                  this.constructFullPermissions(commands, c, guild)
               );
   ...
   async loadGuild(guild, prune = true, createSettings = false) {
      let { modLog, adminRole, modRole, muteRole, crownRole } =
         await this.extractSettings(guild, createSettings);
      ...
      guild.members.fetch().then((members) => {
         ...
         if (prune) {
            ...
            const currentMemberIds = this.db.users.selectCurrentMembers
               .all(guild.id)
               .map((row) => row.user_id);
            ...
            const missingMemberIds = this.db.users.selectMissingMembers
               .all(guild.id)
               .map((row) => row.user_id);
            for (const id of missingMemberIds) {
               if (guild.members.cache.has(id))
                  this.db.users.updateCurrentMember.run(1, id, guild.id);
            }
         }
      });

      await guild.me.setNickname(
         `[${this.db.settings.selectPrefix.pluck().get(guild.id)}] ${this.name}`
      );
   }
